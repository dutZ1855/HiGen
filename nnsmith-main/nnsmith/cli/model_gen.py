...
def main(cfg: DictConfig):
    ...
    original_opset_size = len(opset)
    opset = op_filter(opset, mgen_cfg["include"], mgen_cfg["exclude"])
    if len(opset) == 0:
        MGEN_LOG.warning(
            f"Filtered opset is empty (original size: {original_opset_size}, "
            f"include: {mgen_cfg.get('include')}, exclude: {mgen_cfg.get('exclude')}). "
            f"Falling back to all available operators."
        )
        # Fallback to all available operators
        opset = auto_opset(
            ModelType,
            factory,
            vulops=mgen_cfg["vulops"],
    ...
    # Avoid interactive prompts in non-interactive pipelines (RL/baselines).
    # When enabled, an existing report folder will be removed automatically.
    yes = os.environ.get("NNSMITH_MKDIR_YES", "0").strip().lower() in ("1", "true", "yes", "y")
    mkdir(mgen_cfg["save"], yes=yes)
    if cfg["debug"]["viz"]:
